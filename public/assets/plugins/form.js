...
function NotifyAlert(type, res, msg = null, destination = null) {
    ...
    switch (type) {
        case "error":
            ...
            message = msg ?? res.message ?? res.responseText ?? 'Ops! algo deu errado';
            destination = destination ?? redirect ?? null;
            break;
        case "success":
            background = "linear-gradient(to right, rgb(0, 176, 155), rgb(150, 201, 61))";
            message = msg ?? res.message ?? 'Operação bem sucedida.';
            destination = destination ?? redirect ?? null;
            break;
        case "warning":
            background = "linear-gradient(135deg, rgb(252, 207, 49) 10%, rgb(245, 85, 85) 100%)";
            message = msg ?? res.message ?? res.responseJSON.message ?? 'Aviso! Operação falhou.';
            destination = destination ?? redirect ?? null;
            break;
        default:
...
function ToastAlert(type, res){
    ...
    switch (type) {
        case "error":
            background  = "radial-gradient(circle at 10% 50.5%, rgb(255, 107, 6) 0%, rgb(255, 1, 107) 90%)";
            message     = res ?? 'Ops! algo deu errado';
           
            break;
        case "success":
            background  = "linear-gradient(to right, rgb(0, 176, 155), rgb(150, 201, 61))";
            message     = res ?? 'Operação bem sucedida.';
           
            break;
        case "warning":
            background  = "linear-gradient(135deg, rgb(252, 207, 49) 10%, rgb(245, 85, 85) 100%)";
            message     =    res ?? 'Aviso! Operação falhou.';
