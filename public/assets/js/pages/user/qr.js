...
function createSession() {
	...
	if (attampt == 6) {
		...
		Swal.fire({
			title: 'Opps!',
			text: "Tempo expirado, por favor, recarregue esta página",
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
			confirmButtonText: 'Fechar',
			confirmButtonText: 'Atualizar esta página'
		}).then((result) => {
...
function checkSession() {
	...
	$('.logout-btn').on('click',function(){
		...
		Swal.fire({
			...
			text: "Depois de fazer o logout, você deve fazer o login usando o QR Code",
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
			cancelButtonText: 'Não por favor',
