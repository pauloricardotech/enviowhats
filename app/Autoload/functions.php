...
if (!function_exists('planData')) {
    ...
    function planData($title, $value)
    {
    	if ($title == 'chatbot') {
    		$data['is_bool'] = true;
    		$data['title'] = 'Chatbot (Resposta Automática)';
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);

    		return $data;
    	}
		elseif ($title == 'messages_limit') {
    		$data['is_bool'] = false;
    		$data['title'] = $value == '-1'? 'Limite de mensagens (Ilimitado)':"Limite de mensagens ($value)";
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);

    		return $data;
    	}
		elseif ($title == 'contact_limit') {
    		$data['is_bool'] = false;
    		$data['title'] = $value == '-1'? 'Limite de Contatos (Ilimitado)':"Limite de Contatos ($value)";
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);

    		return $data;
    	}
		elseif ($title == 'device_limit') {
    		$data['is_bool'] = false;
    		$data['title'] = $value == '-1'? 'Limite de Dispositivos (Ilimitado)':"Limite de Dispositivos ($value)";
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);

    		return $data;
    	}
		elseif ($title == 'template_limit') {
    		$data['is_bool'] = false;
    		$data['title'] = $value == '-1'? 'Limite de Templates (Ilimitado)':"Limite de Templates ($value)";
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);

    		return $data;
    	}
		elseif ($title == 'apps_limit') {
    		$data['is_bool'] = false;
    		$data['title'] = $value == '-1'? 'Limite de Apps (Ilimitado)':"Limite de apps ($value)";
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);
    	...
    	elseif ($title == 'bulk_message') {
    		$data['is_bool'] = true;
    		$data['title'] = 'Disparo em massa';
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);
    		return $data;
    	}
    	elseif ($title == 'schedule_message') {
    		$data['is_bool'] = true;
    		$data['title'] = 'Agendamento de mensagens';
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);
    		return $data;
    	}
    	elseif ($title == 'template_message') {
    		$data['is_bool'] = true;
    		$data['title'] = 'Modelos de mensagens';
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);
    		return $data;
    	}
    	elseif ($title == 'access_chat_list') {
    		$data['is_bool'] = true;
    		$data['title'] = 'Acesso a lista de chat';
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);
    		return $data;
    	}
    	elseif ($title == 'access_group_list') {
    		$data['is_bool'] = true;
    		$data['title'] = 'Acesso a lista de grupos';
    		$data['value'] = filter_var($value,FILTER_VALIDATE_BOOLEAN);
    		return $data;
    	}
    	else{
